...
import { IconChamHoi, IconCoinTiktok, IconKeyBoard, IconLanguage, IconLive, IconLogout, IconMailBox, IconMessage, IconMoon, IconStore, IconUser } from "../../../Icons";
    ...
    data: [
      {
        ...
        subItem: [
          {
            level: 2,
            titleHeader: "Languages",
            data: DATA_LANGUAGES,
          },
        ],
      },
      {
        iconleft: <IconChamHoi />,
        title: "Phản hồi và trợ giúp",
        to: "/feedback",
      },
      {
        iconleft: <IconKeyBoard />,
...
const MENU_ITEMS_LOGIN = [
  {
    ...
    data: [
      {
        iconleft: <IconUser ></IconUser>,
        title: "Xem hồ sơ",
        to: "/profile",
      },
      {
        iconleft: <IconCoinTiktok />,
        title: "Nhận xu",
        to: "/coin",
      },
      {
      ...
      },
      {
        ...
        iconright: <FontAwesomeIcon icon={faCircle}  ></FontAwesomeIcon>
      },
      ...MENU_ITEMS_N0_LOGIN[0].data,
      {
        iconleft: <IconLogout />,
        title: "Đăng xuất",
      },
...
export default function Header() {
  ...
  //khi click vào menuItem không có sự kiện
  const clickItemMenuNoEvent = (item, level) => {
    console.log(item, level);
  };
  return (
    <div className={cx("wrapper")}>
      <div className={cx("inner")}>
        <Link to={'/'} className={cx("logo-tiktok")}>
          {/* copy trên tik tok về theo id */}
            ...
            <>
              <Tippy delay={[0, 100]} content="Tin nhắn" placement="bottom">
                <button className={cx("message")}>
                  ...
                  />
                  <span className={cx("notification-count")}>1</span>
                </button>
              </Tippy>
              <Tippy delay={[0, 100]} content="Hộp thư" placement="bottom">
                <button className={cx("mailbox")}>
                  <IconMailBox
                    width="32px"
                    height="32px"
                    className={cx("mailbox-btn")}
                  />
                  <span className={cx("notification-count")}>1</span>
                </button>
              </Tippy>
            ...
            {currentUserLogin ? (
              <button>
                <img
                  ...
                  src={srcAvatarUser}
                  onError={(e) => {
                    console.log((e.target.src = errorSrcImage));
                  }}
                ></img>
              </button>
            ) : (
              <button className={cx("header-option")}>
                <FontAwesomeIcon
                  icon={faEllipsisVertical}
                  className={cx("header-option")}
                ></FontAwesomeIcon>
              </button>
            )}
          </Menu>
